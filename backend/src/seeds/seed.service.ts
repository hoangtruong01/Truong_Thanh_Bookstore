...
export class SeedService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    if (productCount !== 20 || !firstCategory || !firstCategory.name.includes('Combo')) {
      this.logger.log(`Triggering re-seed: product count is ${productCount}, first category is ${firstCategory?.name || 'none'}`);
  ...
  async seed() {
    ...
    const parentCategories = [
      { name: 'Combo Bút - Viết', slug: 'combo-but-viet', description: 'Các loại combo bút viết học sinh và văn phòng', image: '' },
      { name: 'Combo Học Sinh', slug: 'combo-hoc-sinh', description: 'Combo học tập cho học sinh các cấp', image: '' },
      { name: 'Combo Văn Phòng', slug: 'combo-van-phong', description: 'Combo thiết bị và dụng cụ văn phòng chuyên nghiệp', image: '' },
      { name: 'Combo Sổ - Tập - Giấy', slug: 'combo-so-tap-giay', description: 'Giấy in, tập vở học sinh và sổ tay', image: '' },
      { name: 'Combo Vẽ - Thủ Công', slug: 'combo-ve-thu-cong', description: 'Bộ dụng cụ vẽ, màu vẽ nghệ thuật và thủ công', image: '' },
      { name: 'Combo Máy Tính - Thiết Bị', slug: 'combo-may-tinh-thiet-bi', description: 'Máy tính bỏ túi khoa học và các thiết bị hỗ trợ', image: '' },
    ];
    ...
    const subCategories = [
      { name: 'Combo Bút Bi Tiết Kiệm', slug: 'combo-but-bi', parentId: createdParents[0]._id },
      { name: 'Combo Bút Gel Cao Cấp', slug: 'combo-but-gel', parentId: createdParents[0]._id },
      { name: 'Combo Bút Chì Màu', slug: 'combo-but-chi', parentId: createdParents[0]._id },
      { name: 'Combo Bút Dạ Quang', slug: 'combo-but-da-quang', parentId: createdParents[0]._id },
      { name: 'Combo Gôm Tẩy Ngộ Nghĩnh', slug: 'combo-gom-tay', parentId: createdParents[1]._id },
      { name: 'Combo Thước Kẻ Đa Năng', slug: 'combo-thuoc', parentId: createdParents[1]._id },
      { name: 'Combo Hộp Bút Học Sinh', slug: 'combo-hop-but', parentId: createdParents[1]._id },
      { name: 'Combo Balo & Phụ Kiện', slug: 'combo-balo-hoc-sinh', parentId: createdParents[1]._id },
      { name: 'Combo Keo Dán Văn Phòng', slug: 'combo-keo-cat', parentId: createdParents[2]._id },
      { name: 'Combo Dao Rọc Giấy & Kéo', slug: 'combo-dao-roc-giay', parentId: createdParents[2]._id },
      { name: 'Combo Băng Keo Đóng Hàng', slug: 'combo-bang-keo', parentId: createdParents[2]._id },
      { name: 'Combo Kẹp Giấy Xinh Xắn', slug: 'combo-kep-giay', parentId: createdParents[2]._id },
      { name: 'Combo Bìa Hồ Sơ Tiện Lợi', slug: 'combo-bia-ho-so', parentId: createdParents[2]._id },
      { name: 'Combo File Lưu Trữ Tài Liệu', slug: 'combo-file-tai-lieu', parentId: createdParents[2]._id },
      { name: 'Combo Sổ Tay Ghi Chép', slug: 'combo-so-tay', parentId: createdParents[3]._id },
      { name: 'Combo Tập Vở Viết', slug: 'combo-tap-vo', parentId: createdParents[3]._id },
      { name: 'Combo Giấy In A4 Mịn', slug: 'combo-giay-a4', parentId: createdParents[3]._id },
      { name: 'Combo Giấy Note Đa Sắc', slug: 'combo-giay-note', parentId: createdParents[3]._id },
      { name: 'Combo Màu Vẽ Mỹ Thuật', slug: 'combo-mau-ve', parentId: createdParents[4]._id },
      { name: 'Combo Cọ Vẽ & Giá Vẽ', slug: 'combo-co-ve', parentId: createdParents[4]._id },
      { name: 'Combo Casio & Máy Tính', slug: 'combo-may-tinh-bo-tui', parentId: createdParents[5]._id },
    ];
    ...
    const products = [
      ...
      { name: 'Bút bi Thiên Long TL-027', slug: 'but-bi-thien-long-tl-027', sku: 'TL-027', description: 'Bút bi Thiên Long TL-027 nét viết thanh mảnh, mực đều, bền bỉ và kinh tế.', category: findSub('combo-but-bi'), brand: 'Thiên Long', price: 5000, discountPrice: 4000, stock: 500, images: ['https://images.unsplash.com/photo-1583485088034-697b5bc54ccd?w=500'], rating: 4.8, sold: 120, isFeatured: true, status: ProductStatus.ACTIVE },
      // 2. Bút bi Thiên Long TL-095
      { name: 'Bút bi Thiên Long TL-095', slug: 'but-bi-thien-long-tl-095', sku: 'TL-095', description: 'Bút bi bấm cao cấp Thiên Long TL-095 có đệm cao su êm tay khi viết lâu.', category: findSub('combo-but-bi'), brand: 'Thiên Long', price: 12000, discountPrice: 10000, stock: 300, images: ['https://images.unsplash.com/photo-1569003339405-ea396a5a8a90?w=500'], rating: 4.7, sold: 85, isFeatured: true, status: ProductStatus.ACTIVE },
      // 3. Bút gel Uni-ball Signo 0.5mm
      { name: 'Bút gel Uni-ball Signo 0.5mm', slug: 'but-gel-uni-ball-signo-0.5mm', sku: 'UNI-UM151', description: 'Bút gel nước Uni-ball Signo UM-151 nhập khẩu Nhật Bản, nét chữ cực kì sắc sảo.', category: findSub('combo-but-gel'), brand: 'Uni-ball', price: 35000, discountPrice: 32000, stock: 150, images: ['https://images.unsplash.com/photo-1585336261022-680e295ce3fe?w=500'], rating: 4.9, sold: 90, isFeatured: true, status: ProductStatus.ACTIVE },
      // 4. Bút ký cao cấp Pentel EnerGel BL77
      { name: 'Bút ký cao cấp Pentel EnerGel BL77', slug: 'but-ky-cao-cap-pentel-energel-bl77', sku: 'PENTEL-BL77', description: 'Bút ký gel Pentel EnerGel BL77 mực nước mau khô, thiết kế sang trọng lịch lãm.', category: findSub('combo-but-gel'), brand: 'Pentel', price: 48000, discountPrice: 42000, stock: 100, images: ['https://images.unsplash.com/photo-1511556532299-8f662fc26c06?w=500'], rating: 4.9, sold: 65, isFeatured: true, status: ProductStatus.ACTIVE },
      // 5. Bút chì gỗ 2B Thiên Long GP-01
      { name: 'Bút chì gỗ 2B Thiên Long GP-01', slug: 'but-chi-go-2b-thien-long-gp-01', sku: 'GP-01', description: 'Bút chì gỗ 2B cao cấp thích hợp viết vẽ, tô trắc nghiệm nhanh chóng.', category: findSub('combo-but-chi'), brand: 'Thiên Long', price: 5000, discountPrice: 0, stock: 600, images: ['https://images.unsplash.com/photo-1519750783826-e2420f4d687f?w=500'], rating: 4.6, sold: 210, isFeatured: false, status: ProductStatus.ACTIVE },
      // 6. Bút chì kim Pentel AX105
      { name: 'Bút chì kim Pentel AX105', slug: 'but-chi-kim-pentel-ax105', sku: 'PENTEL-AX105', description: 'Bút chì kim bấm học sinh Pentel AX105 thiết kế trẻ trung kèm gôm tẩy tiện lợi.', category: findSub('combo-but-chi'), brand: 'Pentel', price: 25000, discountPrice: 20000, stock: 200, images: ['https://images.unsplash.com/photo-1501504905252-473c47e087f8?w=500'], rating: 4.7, sold: 45, isFeatured: false, status: ProductStatus.ACTIVE },
      // 7. Bút dạ quang Pastel Set 6 màu
      { name: 'Bút dạ quang Pastel Set 6 màu', slug: 'but-da-quang-pastel-set-6-mau', sku: 'HL-PS-6', description: 'Bộ 6 bút dạ quang màu pastel dịu nhẹ bảo vệ mắt, không thấm trang giấy sau.', category: findSub('combo-but-da-quang'), brand: 'Deli', price: 89000, discountPrice: 75000, stock: 120, images: ['https://images.unsplash.com/photo-1513542789411-b6a5d4f31634?w=500'], rating: 4.8, sold: 55, isFeatured: true, status: ProductStatus.ACTIVE },
      // 8. Gôm tẩy Pentel H.03
      { name: 'Gôm tẩy Pentel H.03', slug: 'gom-tay-pentel-h-03', sku: 'PENTEL-H03', description: 'Gôm tẩy siêu sạch Pentel Nhật Bản, tẩy nhẹ nhàng không rách giấy.', category: findSub('combo-gom-tay'), brand: 'Pentel', price: 12000, discountPrice: 10000, stock: 400, images: ['https://images.unsplash.com/photo-1607604276583-eef5d076aa5f?w=500'], rating: 4.7, sold: 130, isFeatured: false, status: ProductStatus.ACTIVE },
      // 9. Thước kẻ nhựa dẻo Maped 30cm
      { name: 'Thước kẻ nhựa dẻo Maped 30cm', slug: 'thuoc-ke-nhua-deo-maped-30cm', sku: 'MAPED-30', description: 'Thước kẻ dẻo cao cấp chống gãy gập hiệu Maped của Pháp.', category: findSub('combo-thuoc'), brand: 'Maped', price: 18000, discountPrice: 15000, stock: 250, images: ['https://images.unsplash.com/photo-1509062522246-3755977927d7?w=500'], rating: 4.5, sold: 80, isFeatured: false, status: ProductStatus.ACTIVE },
      // 10. Hộp bút sắt hai tầng cá tính
      { name: 'Hộp bút sắt hai tầng cá tính', slug: 'hop-but-sat-hai-tang-ca-tinh', sku: 'HB-SAT', description: 'Hộp đựng bút bằng sắt bền đẹp thiết kế 2 tầng nhiều ngăn tiện dụng.', category: findSub('combo-hop-but'), brand: 'Trường Thành', price: 45000, discountPrice: 38000, stock: 80, images: ['https://images.unsplash.com/photo-1544816155-12df9643f363?w=500'], rating: 4.4, sold: 30, isFeatured: false, status: ProductStatus.ACTIVE },
      // 11. Keo dán khô Deli 9g
      { name: 'Keo dán khô Deli 9g', slug: 'keo-dan-kho-deli-9g', sku: 'DELI-A200', description: 'Hồ dán dạng khô Deli 9g nhỏ gọn, độ kết dính cao, sạch sẽ không nhăn giấy.', category: findSub('combo-keo-cat'), brand: 'Deli', price: 8000, discountPrice: 0, stock: 500, images: ['https://images.unsplash.com/photo-1506784983877-45594efa4cbe?w=500'], rating: 4.5, sold: 140, isFeatured: false, status: ProductStatus.ACTIVE },
      // 12. Dao rọc giấy SDI 0439
      { name: 'Dao rọc giấy SDI 0439', slug: 'dao-roc-giay-sdi-0439', sku: 'SDI-0439', description: 'Dao rọc giấy mini SDI thép không gỉ sắc bén, có khóa chốt an toàn.', category: findSub('combo-dao-roc-giay'), brand: 'SDI', price: 22000, discountPrice: 18000, stock: 150, images: ['https://images.unsplash.com/photo-1531346878377-a5be20888e57?w=500'], rating: 4.6, sold: 40, isFeatured: false, status: ProductStatus.ACTIVE },
      // 13. Băng keo trong Deli 2 inch 100y
      { name: 'Băng keo trong Deli 2 inch 100y', slug: 'bang-keo-trong-deli-2-inch-100y', sku: 'DELI-TAP-30', description: 'Băng keo trong bản rộng 4.8cm dày dặn, độ dính cực tốt cho đóng gói thùng.', category: findSub('combo-bang-keo'), brand: 'Deli', price: 25000, discountPrice: 20000, stock: 300, images: ['https://images.unsplash.com/photo-1586075010923-2dd4570fb338?w=500'], rating: 4.4, sold: 110, isFeatured: false, status: ProductStatus.ACTIVE },
      // 14. Kẹp bướm Slecho 25mm hộp 12 cái
      { name: 'Kẹp bướm Slecho 25mm hộp 12 cái', slug: 'kep-buom-slecho-25mm-hop-12-cai', sku: 'SL-KB25', description: 'Kẹp bướm văn phòng Slecho 25mm kẹp giữ tài liệu tài liệu chắc chắn không rách.', category: findSub('combo-kep-giay'), brand: 'Slecho', price: 18000, discountPrice: 15000, stock: 200, images: ['https://images.unsplash.com/photo-1457369804613-52c61a468e7d?w=500'], rating: 4.5, sold: 75, isFeatured: false, status: ProductStatus.ACTIVE },
      // 15. File hồ sơ nhựa nút bấm A4
      { name: 'File hồ sơ nhựa nút bấm A4', slug: 'file-ho-so-nhua-nut-bam-a4', sku: 'FILE-A4', description: 'Bìa hồ sơ lá nhựa trong có nút bấm bảo vệ hồ sơ tài liệu khỏi ẩm ướt.', category: findSub('combo-file-tai-lieu'), brand: 'Trường Thành', price: 6000, discountPrice: 5000, stock: 800, images: ['https://images.unsplash.com/photo-1586075010923-2dd4570fb338?w=500'], rating: 4.3, sold: 340, isFeatured: false, status: ProductStatus.ACTIVE },
      // 16. Giấy A4 Double A 70gsm (500 tờ)
      { name: 'Giấy A4 Double A 70gsm (500 tờ)', slug: 'giay-a4-double-a-70gsm-500-to', sku: 'DA-A4-70G', description: 'Giấy in văn phòng cao cấp Double A Thái Lan định lượng 70gsm láng mịn chống kẹt giấy.', category: findSub('combo-giay-a4'), brand: 'Double A', price: 82000, discountPrice: 78000, stock: 250, images: ['https://images.unsplash.com/photo-1603481588273-2f908a9a7a1b?w=500'], rating: 4.8, sold: 190, isFeatured: true, status: ProductStatus.ACTIVE },
      // 17. Giấy A4 PaperOne 80gsm (500 tờ)
      { name: 'Giấy A4 PaperOne 80gsm (500 tờ)', slug: 'giay-a4-paperone-80gsm-500-to', sku: 'PO-A4-80G', description: 'Giấy in PaperOne định lượng 80gsm siêu trắng, chuyên dùng cho máy in laser tốc độ cao.', category: findSub('combo-giay-a4'), brand: 'PaperOne', price: 98000, discountPrice: 92000, stock: 180, images: ['https://images.unsplash.com/photo-1586075010923-2dd4570fb338?w=500'], rating: 4.9, sold: 120, isFeatured: true, status: ProductStatus.ACTIVE },
      // 18. Sổ tay lò xo Trường Thành A5 120tr
      { name: 'Sổ tay lò xo Trường Thành A5 120tr', slug: 'so-tay-lo-xo-truong-thanh-a5-120tr', sku: 'TT-NB-A5', description: 'Sổ tay bìa da lò xo Trường Thành khổ A5, ruột kẻ ngang chống nhòe mực.', category: findSub('combo-so-tay'), brand: 'Trường Thành', price: 28000, discountPrice: 24000, stock: 150, images: ['https://images.unsplash.com/photo-1544816155-12df9643f363?w=500'], rating: 4.6, sold: 80, isFeatured: true, status: ProductStatus.ACTIVE },
      // 19. Vở học sinh 4 ô ly Hồng Hà 96tr
      { name: 'Vở học sinh 4 ô ly Hồng Hà 96tr', slug: 'vo-hoc-sinh-4-o-ly-hong-ha-96tr', sku: 'HH-TAP-96', description: 'Vở viết học sinh Hồng Hà 4 ô ly định lượng cao, bìa in tranh ngộ nghĩnh.', category: findSub('combo-tap-vo'), brand: 'Hồng Hà', price: 8000, discountPrice: 7000, stock: 1000, images: ['https://images.unsplash.com/photo-1544816155-12df9643f363?w=500'], rating: 4.5, sold: 450, isFeatured: false, status: ProductStatus.ACTIVE },
      // 20. Máy tính Casio FX-580VN X
      { name: 'Máy tính Casio FX-580VN X', slug: 'may-tinh-casio-fx-580vn-x', sku: 'CS-FX580', description: 'Máy tính khoa học thế hệ mới Casio FX-580VN X hỗ trợ đắc lực giải toán thi THPT Quốc Gia.', category: findSub('combo-may-tinh-bo-tui'), brand: 'Casio', price: 690000, discountPrice: 620000, stock: 50, images: ['https://images.unsplash.com/photo-1519389950473-47ba0277781c?w=500'], rating: 4.9, sold: 95, isFeatured: true, status: ProductStatus.ACTIVE }
    ];
