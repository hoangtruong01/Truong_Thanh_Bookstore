...
export class LandingPageService {
  ...
  async generateLandingPage(dto: GenerateLandingPageDto): Promise<any> {
    ...
    if (!apiKey) {
      this.logger.warn('GEMINI_API_KEY not configured. Using fallback template.');
      return this.generateFallbackTemplate(dto);
    }

    this.logger.log(`Calling Gemini AI for landing page generation: ${dto.title}`);
...
`;
    ...
    try {
      ...
      const cleanJson = generatedText.trim().replace(/^```json\n?/, '').replace(/\n?```$/, '').trim();
      return JSON.parse(cleanJson);
    } catch (error) {
      this.logger.warn(`Gemini API error (${error.message}). Falling back to premium template...`);
      return this.generateFallbackTemplate(dto);
    }
  }
  ...
  private generateFallbackTemplate(dto: GenerateLandingPageDto): any {
    ...
    if (promptLower.includes('xanh') || promptLower.includes('blue')) primaryColor = '#0284c7';
    else if (promptLower.includes('lá') || promptLower.includes('green')) primaryColor = '#16a34a';
    else if (promptLower.includes('vàng') || promptLower.includes('cam')) primaryColor = '#ea580c';
    else if (promptLower.includes('tím') || promptLower.includes('purple')) primaryColor = '#7c3aed';
    ...
    const benefits = isEducation ? [
      { title: 'Nội Dung Học Tập Sinh Động', description: 'Hình ảnh màu sắc rõ nét giúp bé say mê tiếp thu kiến thức tự nhiên, không gây nhàm chán.', icon: 'AcademicCapIcon' },
      { title: 'Chất Liệu Giấy Chống Lóa Mắt', description: 'Sử dụng chất liệu giấy chuyên dụng cao cấp chống mỏi mắt, bảo vệ tối đa thị lực của bé.', icon: 'ShieldCheckIcon' },
      { title: 'Quà Tặng Ebook Bổ Trợ Tư Duy', description: 'Tặng kèm tài liệu điện tử độc quyền cùng các bài ôn tập nâng cao tư duy logic và ngôn ngữ.', icon: 'GiftIcon' },
    ] : [
      { title: 'Chất Liệu Cao Cấp & An Toàn', description: 'Sản xuất từ vật liệu an toàn tuyệt đối cho bé, bền đẹp và thân thiện với môi trường.', icon: 'CheckBadgeIcon' },
      { title: 'Thiết Kế Thông Minh Đa Năng', description: 'Hỗ trợ đắc lực cho trẻ trong quá trình học tập, rèn luyện sự tự lập ngăn nắp.', icon: 'LightBulbIcon' },
      { title: 'Cam Kết Đổi Trả 1 Đổi 1', description: 'Hỗ trợ kiểm tra hàng thoải mái trước khi nhận, đổi trả trong vòng 7 ngày nếu lỗi từ nhà sản xuất.', icon: 'ArrowPathIcon' },
    ];
    ...
    return {
      ...
      benefits,
      packages: [
        { name: 'Mua Lẻ 1 Sản Phẩm', price: retailPrice, originalPrice, badge: 'Gói Tiết Kiệm', isBestSeller: false },
        { name: 'Combo 2 Sản Phẩm (Tặng Ebook)', price: Math.round(retailPrice * 2 * 0.85), originalPrice: originalPrice * 2, badge: 'Khuyên Dùng', isBestSeller: true },
        { name: 'Combo 3 Sản Phẩm (Tặng Quà Đặc Biệt)', price: Math.round(retailPrice * 3 * 0.75), originalPrice: originalPrice * 3, badge: 'Ưu Đãi Lớn Nhất', isBestSeller: false },
      ],
      testimonials: [
        { authorName: 'Chị Nguyễn Thị Mai', avatar: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=150', content: 'Mình đã đặt mua combo sản phẩm này cho con. Chất lượng hoàn thiện tốt vượt mong đợi!', rating: 5 },
        { authorName: 'Anh Trần Văn Hùng', avatar: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150', content: 'Shop giao hàng nhanh, được kiểm tra thoải mái. Khuyên mọi người nên mua combo!', rating: 5 },
      ],
      customCss: '',
      isFallback: true,
      aiWarning: 'Đang dùng giao diện mẫu do chưa cấu hình GEMINI_API_KEY. Lấy API Key miễn phí tại aistudio.google.com/apikey và thêm vào file .env.',
    };
  }
}
